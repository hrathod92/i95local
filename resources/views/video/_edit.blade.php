...
@if($user->role == "admin")
    ...
    {!! Form::select( 'company_id', \App\Company::orderBy( 'id' )->pluck( 'title', 'id' )->prepend( 'None', 0 ), isset( $video->company_id ) ? $video->company_id : '' ) !!}
@else
    {!! Form::hidden('company_id', isset($video->company_id) ? $video->company_id : $user->company_id) !!}
@endif

{!! Form::Label( 'video_type_id', 'Type' ) !!}
{!! Form::select( 'video_type_id', \App\VideoType::orderBy( 'id')->pluck( 'title', 'id' ), isset( $video->video_type_id ) ? $video->video_type_id : '' ) !!}

@if ( $user->role == 'admin' )
  {!! Form::Label( 'favorite_id', 'Favorite' ) !!}
  {!! Form::select( 'favorite_id', \App\Favorite::orderBy( 'id')->pluck( 'title', 'id' ), isset( $video->favorite_id ) ? $video->favorite_id : '' ) !!}
@endif
...
{!! Form::select( 'status_id', \App\Status::orderBy( 'id')->pluck( 'title', 'id' ), isset( $video->status_id ) ? $video->status_id : '' ) !!}
