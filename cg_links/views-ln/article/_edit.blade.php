...
<div class="form-element">
  ...
  {!! Form::text( 'contact_us_url', isset($article) && !empty($article->contact_us_url) ? $article->contact_us_url : !empty(\App\Company::where('id', $user->company_id)->first()->contact_us_url) ? \App\Company::where('id', $user->company_id)->first()->contact_us_url : null,[ 'id'=>'url' ]) !!}
</div>
...
<div class="form-element">
  ...
  <span id="spnCharLeft" class="pull-right"></span>
</div>
	...
	$('#meta_description').keyup(function() {
	  ...
	  if(length <= 158){
		  var remain = maxLength-length;
	  	  $('#spnCharLeft').text(remain+ ' characters of 158');
		  $('#spnCharLeft').css('color', 'black');
	  }else{
		  var overage = parseInt(length)-158;
		  $('#spnCharLeft').text(overage+ ' characters over the limit');
		  $('#spnCharLeft').css('color', 'red');
	  }
	});
</script>
